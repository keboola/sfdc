<?php
/**
 * Fixed Zend_Rest_Route
 * http://framework.zend.com/issues/browse/ZF-9222
 *
 * Added subresources support
 *  - inspired by http://framework.zend.com/issues/browse/ZF-11327
 *  - Added: METHOD	/api/users/:id/:resource -> RestController::methodResourceAction
 *
...
namespace Keboola\Rest;

class Route extends \Zend_Rest_Route
{
	public function match($request, $partial = false)
   {
       ...
       if ($path != '') {
		...
		if ($pathOrig != '' || $this->_allRestful()) {
			...
			if (isset($params['resource']) && isset($values[$this->_actionKey])) {
				$values[$this->_actionKey] .= '-' . $params['resource'];
			}
